...
def dist_occur(output_folder):
    ...
    df_by_gene = all_mutations.groupby(['chrom', 'pre_name', 'id', 'stop_pre_build', 'start_pre_build']).agg(
        {'indiv_name': ['nunique',
                        'count'],
         'pos': 'nunique'
         }).reset_index()
    df_by_gene.columns = ['chrom', 'pre_name', 'id', 'stop_pre_build', 'start_pre_build', 'indiv_name_nunique',
